#!/bin/bash

# cd into deploy, in case the script is called from the root project dir
cd deploy

# install terser for minify script
if [[ ! $(which terser) ]]; then
	npm install -g terser
	echo "Installed terser."
fi

# do a release build
cd ..
dub build -b release --compiler ldc2
echo "Built."
cd deploy

# copy needed files to this directory (deploy/)
cp -r ../public ./
cp ../sorcerio ./

# minify
./minify.sh
echo "Minified."

# package the build
tar -czf sorcerio.tar.gz ./public/ ./sorcerio
echo "Packaged."

# Deploy
SORCERIO_DEPLOY_IP="192.81.210.68"
SORCERIO_SSH_USER="sorcerio"

openssl aes-256-cbc -K $encrypted_85a0c774629c_key -iv $encrypted_85a0c774629c_iv -in private.key.enc -out ~/.ssh/id_rsa -d
echo "$SORCERIO_DEPLOY_IP ssh-rsa " >> ~/.ssh/known_hosts # generate with ssh-keyscan -t rsa ip_of_vps
scp sorcerio.tar.gz $SORCERIO_SSH_USER@$SORCERIO_DEPLOY_IP:~/sorcerio/
ssh $SORCERIO_SSH_USER@$SORCERIO_DEPLOY_IP "rm -rf ~/sorcerio/sorcerio ~/sorcerio/public ; cd ~/sorcerio ; tar -xf sorcerio.tar.gz ; rm sorcerio.tar.gz ; systemctl --user restart sorcerio.service"
echo "Deployed."
