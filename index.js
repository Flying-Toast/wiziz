...
server.io.on('connection', function(socket) {
	...
	socket.on('input', function(inputs) {
		...
		if (player) {
			player.quedInputs = player.quedInputs.concat(inputs);
		}
	});
...
setInterval(physicsLoop, 20); //this delay is probably faster than it needs to be
